...
- *Stationary*: a population quantity is stationary (or exhibits stationarity) if it does not change as a function of time. If a time series is second-order stationary, then the population mean $\mu$, the population variance $\sigma^2$, and the population autocorrelation $\rho_k$ are assumed constant as a function of $t$.
- *Lag*: denoted by $k$, it is the number of periods between observations. If, e.g., $k = 1$, then for the `maine.ts` data in particular we are looking at the correlation between one observation and one 12 months in the future, since `frequency=12`. ($k$ need not be integer! For instance, a lag of $k = 1/12$ denotes a lag of one month, i.e., the lag between one bin and the next.) (*NOTE*: this is only strictly true if you pass a `ts` object to `acf()` and plot it. If, on the other hand, you decompose the `ts` object and, e.g., pass the random component to `acf()`, a plot is made instantly, and a lag of 1 maps to a one-bin difference in the random component: for `maine.ts`, this means lag 1 represents one month. Ah, the joys of `R`.)
...
```{r fig.height=4,fig.width=4,fig.align="center"}
a = acf(maine.ts)       # no plot -- ts object
names(a)
plot(a)                 # lag maps to frequency -> lag 1 == 1 year

random.comp = decompose(maine.ts)$random
random.comp = random.comp[!is.na(random.comp)]
a = acf(random.comp)    # plot -- not a ts object -- lag maps to bins -> lag 1 == 1 month
```

The plot is dubbed a *correlogram* and the blue dashed lines are drawn at autocorrelation values of $(-1/n) \pm (2/\sqrt{n})$. Values that lie outside the lines are assumed "significant": we reject the null hypothesis that the true value of the $\rho_k$ is zero. (The estimator is biased, hence the $-1/n$ in the definition of the blue dashed lines.) Here, it appears that the random data $z_t$ are positively and negatively correlated on various timescales less than one year (and perhaps on a 15-month timescale). If you are satisfied with this as an analyst, then your next step is modeling this structure.

Interpreting a correlogram is an art. Here, we might be leery, since the results could, instead of reflecting some real dependance, instead reflect a sub-optimally modeled trend ($m_t$), etc. (Seasonality appears to be well-modeled, however, given the near-zero correlogram value at lag 12 months). 

There are often no easy answers with correlograms!
