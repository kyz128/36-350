...
```{r linewidth=80}
readLines("https://raw.githubusercontent.com/kyz128/36-350/main/README.md")
```
...
source_url("https://raw.githubusercontent.com/kyz128/36-350/main/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/kyz128/36-350-Fork/main/check_mark.R")
```
---
...
f = function(x) { 
  if (typeof(x) != "character"){warning("Input not of character type")}
  toupper(x) 
}
f(5)
```
```
I was actually expecting an error but it seems that R
implicitly converted the int to a char. 
```
...
```{r error=TRUE,linewidth=80}
f = function(x) { 
  if (typeof(x) != "character"){stop("Input not of character type")}
  toupper(x) 
}
f(5)
```
...
```{r error=TRUE,linewidth=80}
f = function(x) {
  message("Function converts x to uppercase when called properly")
  if (typeof(x) != "character"){
    stop("Input not of character type")}
  toupper(x) 
}
f(2)
...
f = function(x) { 
  tryCatch({
    if (x < 0) stop("Negative input; out of natural log's domain")
    if (x == 0) warning("Input is 0, output should be indeterminate")
    log(x)},
...
test_that(desc = "Test for string of length greater than 1",
          expect_warning(f("aa")))
test_that(desc = "Test uppercase input", expect_equal(f("A"), 0))
test_that(desc = "Test lowercase input", expect_equal(f("a"), 0))
test_that(desc = "Test non-char input", expect_error(f(5)))
test_that(desc = "Test input more than 1 string", 
...
```{r linewidth=80}
f = function(letter) {
  if ( typeof(letter) != "character" ) stop("Input type must be character.")
  if ( length(letter) != 1 ) stop("Input must be a string.")
  if ( nchar(letter) > 1 ) {
   letter = letter[1]
   warning("More than 1 letter. Only using the first letter of input.")
  }
